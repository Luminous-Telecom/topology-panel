...
describe('hostNodeFill', () => {
  ...
  it('problema Zabbix sem lastvalue não pinta alerta', () => {
    const metadata: HostMetadataMap = { 'rb-01': { name: 'rb-01', hostid: 'hid1' } };
    const problems = { hid1: { count: 1, maxSeverity: 4 } };
    expect(
      hostNodeFill(node({ zabbixHost: 'rb-01' }), options, metadata, {}, identity, problems)
    ).toBe(options.colorUnknown);
  });

  it('lastvalue 0 pinta offline mesmo se o display ainda disser online', () => {
    const display: HostDisplayMap = {
      'rb-01': { status: 'online', color: options.colorOnline, value: 0 },
    };
    const metadata: HostMetadataMap = { 'rb-01': { name: 'rb-01', hostid: 'hid1' } };
    const problems = { hid1: { count: 1, maxSeverity: 4 } };
    expect(
