...
describe('shouldApplyNavigationFit', () => {
  it('aplica na primeira vez e ao trocar de mapa', () => {
    expect(shouldApplyNavigationFit(null, 'root:win', 800, 600)).toBe(true);
    const prev = { navKey: 'root:win', w: 800, h: 600 };
    expect(shouldApplyNavigationFit(prev, 'apodi:win', 800, 600)).toBe(true);
  });

  it('não reaplica no mesmo mapa quando o viewport não cresceu (refresh / zoom)', () => {
    const prev = { navKey: 'apodi:win', w: 800, h: 600 };
    expect(shouldApplyNavigationFit(prev, 'apodi:win', 800, 600)).toBe(false);
    expect(shouldApplyNavigationFit(prev, 'apodi:win', 785, 600)).toBe(false);
  });

  it('reaplica quando o painel cresce (maximizar o card)', () => {
    const prev = { navKey: 'apodi:win', w: 800, h: 200 };
    expect(shouldApplyNavigationFit(prev, 'apodi:win', 800, 700)).toBe(true);
  });
});
...
describe('computeMapScrollMetrics', () => {
  ...
  it('pan além da origem do conteúdo (mapa centralizado) satura scrollLeft/scrollTop em 0', () => {
    const centered = { x0: -48, y0: -48, x1: 2048, y1: 1548, width: 2096, height: 1596 };
    const metrics = computeMapScrollMetrics(centered, { x: 100, y: 80, scale: 1 }, 1200, 800);
