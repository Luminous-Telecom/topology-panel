...
/** Problemas ativos Zabbix para badges NOC — não usado para cor/status do mapa. */
export function useZabbixHostProblems(
  ...
  useEffect(() => {
    ...
    void problemsCache
      .get(`${datasourceUid}\u0000problems\u0000${hostKey}`, () =>
        fetchZabbixHostProblems(datasourceUid, hostKey.split('\0'))
      )
      .then((next) => {
        if (!cancelled) {
          setProblems(next);
